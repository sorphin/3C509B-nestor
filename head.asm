...
; effect we can't send type 8137, and it will only confuse Netware.
	...
	jne	recv_not_8137		;no, don't change it.
	mov	word ptr es:[di],word ptr 3881h	;yes, mung it slightly.
recv_not_8137:
not_n_op:

	mov	bx,offset type_list	;point bx to the ordered list.
recv_locate_1:
	mov	bx,[bx].next_type	;point to the next one.
	cmp	bx,0			;any more handles to look at?
	je	recv_locate_4		;no - can't be for us.

	mov	ax,[bx].receiver.offs	;do they have a receiver?
	or	ax,[bx].receiver.segm
	je	recv_locate_1		;no - they're not serious about it.
...
;without breaking BLUEBOOK type length=0 clients.
	...
	jne	recv_locate_2		;no.
	cmp	dl,IEEE8023		;is the packet also IEEE 802.3?
	jne	recv_locate_1		;no, give up on it now.
recv_locate_2:

	mov	cx,[bx].packet_type_len	;compare the packets.
	lea	si,[bx].packet_type
	jcxz	recv_locate_3		;if cx is zero, they want them all.

	cmp	[bx].class, dl		;is this the right class?
	jne	recv_locate_1		;no- don't bother

	push	di
	repe	cmpsb
	pop	di
	jne	recv_locate_1		;types don't match.
recv_locate_3:
